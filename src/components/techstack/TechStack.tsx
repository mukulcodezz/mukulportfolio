...
export default function TechStack() {
  ...
  return (
    <SectionWrapper id="techstack" direction="right">
            ...
            >
              ...
              <div className="flex flex-wrap gap-1.5">
                {col.items.map((item) => (
                  <motion.span
                    key={item}
                    whileHover={shouldReduce ? undefined : { scale: 1.06, borderColor: 'rgba(16,185,129,0.4)', color: '#ededed' }}
                    transition={{ type: 'spring', stiffness: 400, damping: 20 }}
                    className="text-mono text-[11px] px-2 py-1 rounded border border-line text-text cursor-default"
                  >
                    {item}
                  </motion.span>
                ))}
